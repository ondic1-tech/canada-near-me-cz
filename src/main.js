...
const searchRun = await Actor.call('apify/google-search-scraper', {
  queries: queries.map((query) => `${query} site:.cz`).join('\n'),
  countryCode: 'cz',
  languageCode: 'cs',
  maxPagesPerQuery: 2,
  resultsPerPage: 100,
...
const { items: searchPages } = await searchDataset.getData({ limit: 100 });
const discoveredUrls = searchPages.flatMap((page) => page.organicResults ?? []).map((result) => result.url).filter(Boolean);
const requests = [...new Set(discoveredUrls)].filter(allowedHost).slice(0, maxProductPages).map((url) => ({
  url,
...
const crawler = new CheerioCrawler({
  ...
  async requestHandler({ request, $, requestQueue }) {
    ...
    if (label === 'STORE') {
      ...
      await Actor.pushData({ ...candidate, branch_address: address, latitude: coordinates?.latitude ?? null, longitude: coordinates?.longitude ?? null, map_ready: Boolean(coordinates), branch_evidence_url: request.url });
      return;
    }
    ...
    await Actor.pushData({ ...base, branch_address: null, latitude: null, longitude: null, map_ready: false, branch_evidence_url: request.url });
  },
